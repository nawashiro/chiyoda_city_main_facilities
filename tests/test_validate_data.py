...
class ValidateKeyLocationsTests(unittest.TestCase):
    ...
    def test_phase_zero_documents_exist_and_plan_stays_in_scope(self):
        ...
        for document in documents:
            ...
            self.assertIn("id + name + coordinates", document)
            self.assertIn("id + name + qid", document)
            self.assertIn("WAM、OSM、検索入力座標", document)
            self.assertIn("単一`name`", document)
            self.assertIn("別の状態台帳", document)
            self.assertNotIn('"names"', document)
