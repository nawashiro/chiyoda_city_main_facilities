...
既知ID、固定の千代田区包含bboxに限定したQID、各座標検索入力の`around:50`を単一Overpass queryで取得する。カテゴリでは絞り込まない。行政境界areaの解決はgateway timeoutを招くため行わない。施設ごとのN+1問い合わせは行わない。
...
定期cronは同一ソースを30日より短い間隔で取得しない。手動の保守・修復取得はこの間隔に拘束されず、取得処理は前回取得時刻を理由に拒否しない。運用目安はWAMが公式公開後の年2回、OSMと町名が四半期または必要時である。
